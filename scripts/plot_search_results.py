...
def load_progress(output_dir: Path) -> List[Dict[str, Any]]:
    p = output_dir / "rmse_progress.json"
    if not p.exists():
        raise FileNotFoundError(f"rmse_progress.json not found in {output_dir}")
    return json.loads(p.read_text())


def load_identifiability(output_dir: Path) -> List[Dict[str, Any]]:
    files = sorted(output_dir.glob("identifiability_month_*.json"))
    return [json.loads(f.read_text()) for f in files]


def load_accepted_months(output_dir: Path) -> List[Dict[str, Any]]:
    files = sorted(output_dir.glob("accepted_month_*.json"))
    return [json.loads(f.read_text()) for f in files]
...
def plot_param_evolution(records: List[Dict], accepted: List[Dict], ax):
    ...
    at the end of each month (read from rmse_progress.json best-per-month).
    """
    if not records:
    ...
    # gather param_* keys
    param_cols = sorted({k for r in records for k in r.keys()
                         if k.startswith("param_")})
    if not param_cols:
        ax.text(0.5, 0.5, "No param snapshots recorded", ha="center", va="center",
                transform=ax.transAxes)
        ax.set_title("Accepted param evolution")
        return

    # best record per month
    months_seen = sorted({r.get("month", 1) for r in records})
    best_per_month = {}
    for m in months_seen:
        candidates = [r for r in records
                      if r.get("month") == m and r.get("combined_rmse") is not None]
        if not candidates:
            continue
        best = min(candidates, key=lambda r: _finite(r.get("combined_rmse", float("inf"))))
        best_per_month[m] = best

    # group param cols by temporal path prefix
    prefix_map: Dict[str, List[str]] = {}
    for col in param_cols:
        # col like "param_infection_modulation_b3"
    ...
    for pi, (prefix, cols) in enumerate(sorted(prefix_map.items())):
        ...
        for ci, col in enumerate(cols_sorted):
            ...
            label = f"{TEMPORAL_SHORT.get(prefix, prefix)[:10]} b{ci}" if ci == 0 or ci == n_buckets-1 else None
            ax.plot(xs, ys, "-o", markersize=4, color=color,
                    alpha=0.7, linewidth=1, label=label)

    ax.set_title("Accepted config – temporal param values (best/month)")
    ax.set_xlabel("Month")
