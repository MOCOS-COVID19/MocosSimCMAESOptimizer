...
@testset "twelve-month Saxony production pilot" begin
    ...
    @test occursin("#SBATCH -c 4", wrapper)
    @test occursin("#SBATCH --mem=20G", wrapper)
    @test occursin("SLURM_SUBMIT_DIR", wrapper)
    @test occursin("Cannot locate optimizer Project.toml", wrapper)
    @test findfirst("SLURM_SUBMIT_DIR", wrapper) < findfirst("BASH_SOURCE", wrapper)
